...
!function(){var a="img/icons/";d3.csv("data.csv",function(b){console.log(b),d3.csv("organs.csv",function(c){function d(a){j.selectAll("path").classed("selected",!1).data(a,function(a){return a.key}).classed("selected",!0)}function e(){var a=[];return _.each(b,function(b){_.each(b.organs,function(c){a.push({pos:[b.bb,c.bb],key:b.name+b.index+c.organ})})}),a}console.log(c),_.each(b,function(a){a.organs=a.organs.split(", ");for(var b=0;b<a.organs.length;b++){var d=_.findWhere(c,{organ:a.organs[b]});d?(a.organs[b]=d,d.vitamins||(d.vitamins=[]),d.vitamins.push(a)):console.log("problem",a.organs[b])}});var f=d3.select(".content .vitamins").selectAll(".vitamin").data(b,function(a){return a.name+a.index}).enter().append("div").attr("class","vitamin btn").on("click",function(a){d3.select(".content .vitamins").selectAll(".vitamin").classed("selected",!1),d3.select(this).classed("selected",!0),d3.select(".content .organs").selectAll(".organ").classed("selected",!1).data(a.organs,function(a){return a.organ}).classed("selected",!0);var b=[];_.each(a.organs,function(c){b.push({key:a.name+a.index+c.organ})}),d(b)}),g={w:86,h:29};f.append("svg").attr("height",g.h).attr("width",g.w).append("path").attr("class","bgr").attr("d","M68.6,0c0.2,0,0.4,0.1,0.5,0.2c0.1,0.1,0.2,0.2,0.2,0.4c3.7,5.5,8.8,9.9,14.9,12.7c0.6,0.3,0.8,0.4,1.2,0.5c0,0,0,0,0,0c0.3,0.1,0.5,0.4,0.5,0.8c0,0.3-0.2,0.5-0.4,0.7c-0.1,0-0.2,0.1-0.2,0.1c-0.4,0.2-0.7,0.3-1.1,0.5c-6.1,2.8-11.2,7.2-14.9,12.7c-0.1,0.1-0.2,0.2-0.2,0.4C69,28.9,68.8,29,68.6,29H0V0H68.6z"),f.append("div").text(function(a){return a.name+a.index}).attr("class","name"),f.each(function(a){var b=this.getBoundingClientRect();a.bb={x:b.left+g.w,y:b.top+g.h/2}});var h=d3.select(".content .organs").selectAll(".organ").data(c,function(a){return a.organ}).enter().append("div").attr("class","organ btn").on("click",function(a){d3.select(".content .organs").selectAll(".organ").classed("selected",!1),d3.select(this).classed("selected",!0),d3.select(".content .vitamins").selectAll(".vitamin").classed("selected",!1).data(a.vitamins,function(a){return a.name+a.index}).classed("selected",!0);var b=[];_.each(a.vitamins,function(c){b.push({key:c.name+c.index+a.organ})}),d(b)}),i={w:45,h:51};h.append("svg").attr("height",i.h).attr("width",i.w).append("polygon").attr("class","bgr").attr("points","44.4,38.5 22.2,51.3 0,38.5 0,12.8 22.2,0 44.4,12.8"),h.append("img").attr("src",function(b){return a+b.img}).attr("class","pic").attr("height",i.w).attr("width",i.w),h.append("div").text(function(a){return a.organ}).attr("class","name"),h.each(function(a){var b=this.getBoundingClientRect();a.bb={x:b.left,y:b.top+i.h/2}});var j=d3.select(".graph"),k=j[0][0].getBoundingClientRect(),l=d3.svg.diagonal().source(function(a){return{x:a.pos[0].y-k.top,y:a.pos[0].x-k.left}}).target(function(a){return{x:a.pos[1].y-k.top,y:a.pos[1].x-k.left}}).projection(function(a){return[a.y,a.x]}),m=e();j.selectAll("path").data(m,function(a){return a.key}).enter().append("path").attr("class","horde").attr("d",l)})})}();